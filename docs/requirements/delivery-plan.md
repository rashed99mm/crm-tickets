...
| Sprint | Name | Slice | FEAT | Stories | Points | Status |
|---|---|---|---|---|---|---|
| 16 | Communication channels — WhatsApp, SMS conversations, live chat, web forms | S5 (reopened) | `FEAT-24`–`FEAT-27` | US-230–US-240 (to be filed alongside the spec) | — | spec + backend plan + tasks written 2026-08-27. **Verified 2026-09-02 by running the suite, not by reading it:** shared ingestion (`CC-1`–`CC-5`) and WhatsApp inbound (`CC-6`–`CC-9`) are built and green. **Outbound is broken on every channel** — `CC-10` is a red test and `CC-13` shares the same defect (spec `A19`: the credential is decrypted twice, so no request ever leaves the process; fix is `CC-51`). Live chat partial; **SMS inbound (`CC-11`/`CC-12`) and the web-form backend (`CC-20`–`CC-23`) not built**. Production deployment still blocked on `A11` (`OQ-CC-1..3`) |
| 22 | Mock provider gateway + mock/real toggle — one provider-faithful mock per channel, inbound email | EPIC-03 (cont.) | `FEAT-35` | `CC-30`–`CC-51` (spec amendments 2026-09-02) | — | **implemented and verified 2026-09-02, uncommitted** across two plans. Plan 1 ([record](../superpowers/plans/EPIC-03-US-201-feat-35-channel-mock-gateway/README.md)): provider-faithful mocks (SendGrid/Meta/Twilio shapes), the `Channels:UseMocks` toggle, `CC-48` one channel allow-list, `CC-49` one sender base, `CC-51` the outbound-credential fix. Plan 2 ([record](../superpowers/plans/EPIC-03-US-201-feat-35-inbound-completion/README.md)): SMS inbound with Twilio signature verification (`CC-40`/`CC-41`), inbound email (`CC-42`/`CC-43`), the email reply path (`CC-44`), the portal web-form backend (`CC-47`). **`CC-45`/`CC-46` (live-chat visitor simulator, abandoned-session job) are deferred by explicit instruction (2026-09-02)** — not forgotten; they need their own plan. Frontend: the web form's portal screen already existed (`portal-app/features/web-form`), so plan 2's web-form work was backend-only by that fact, not by omission. `FEAT-35` because 34 was the highest number in use |
...
| Sprint | Name | Slice | FEAT | Stories | Points | Status |
|---|---|---|---|---|---|---|
| 21 | Role & permission workbench — staged editing, atomic per-role saves, global confirmation | EPIC-09 (administration) | `FEAT-34` | US-806–US-807 | 18 | stories filed 2026-09-01 · spec approved 2026-09-01 · backend + frontend plans written and **implemented** the same day ([`EPIC-09-US-806-feat-34-permissions-workbench/`](../superpowers/plans/EPIC-09-US-806-feat-34-permissions-workbench/implementation-plan.md)). Frontend fully verified (104 tests across `permissions.component`, the new `permissions-dirty.guard`, `confirmation.service`/`confirmation-host`, and `users`/`departments`/`sla-policies` adoption). Backend (`AC-806.1`–`AC-806.10`) is unit-tested (13/13) but integration verification is **blocked** by a pre-existing sandbox defect in permission/identity seeding, reproduced on an untouched pre-existing test in isolation — not a defect in this feature; see the plan's README. |
